...
class Analyser():
    ...
    def parse_graph(self,myGraph):
        for node in myGraph.nodes():
            ...
            nType = myGraph.node[node]['nType']
            node = {'id':str(node),'x':xyz[0],'y':xyz[1],'z':xyz[2],'nType':nType}
    ...
    def create_mesh(self,name='xxx'):
        if name == 'xxx':
            filename = "xxx."+str(Analyser.counter)+".mesh"
            Analyser.counter +=1
        else:
            filename=name+'.mesh' 
    ...
    def create_debug(self):
        ...
        for node in self.nodeList:
            mesh.write('node '+node['id']+' '+str(node['x'])+' '+str(node['y'])+' '+str(node['z'])+' '+node['nType']+'\n')
        for edge in self.edgeList:
    ...
    def apply_stresses(self):        
        ...
        for node in self.nodeList:
            ...
            if node['nType'] == 'handrail':
                self.loadNodes.append(int(node['id']))
            #if node['nType'] == 'walkway':
            #    self.loadNodes.append(int(node['id']))
        #print "fixed:",len(self.fixedList)
        #print "loaded",len(self.loadNodes)

        #SLFFEA applies load to edges, find edges connecting loadNodes
        for edge in self.edgeList:
            ptA,ptB = int(edge['ptA']),int(edge['ptB'])
            if ptA in self.loadNodes and ptB in self.loadNodes:
