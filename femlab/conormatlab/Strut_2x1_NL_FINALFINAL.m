...
force(10)=-1;
force(5) = -0.1; % Nominal Horz. Load to encourage buckling (10% of Vertical)
...
iterations = 500;        % Number of iterations per phase (number of loops run)
...
i_mean = 30e-6*ones(1,numberElements);    % means of areas (take inital min_areas as mean????)
i_std  = 0.2*i_mean;                     % variance of areas (0.1 is correct????)
i_mean_reset = 30e-6*ones(1,numberElements); 
i_std_reset = 0.25*i_mean_reset;
std_check_limit = 1e-9;
nl_plot = 5;                            % Initalising fig. counter for NL Plotting (starts at 5 at increases by 1 per plot)
areas_limit = 1e-7;                   % setting min areas so if lower converted to near zero value
...
max_limit_pass = 50;                   % Setting max. no of convergence resets of std. deviation
...
if j > 2     
    % Creating if condition that the top performing solution from the previous iteration is brought forward as 1st member of population 
 if i == 1
   areas(i,:) = fittest_areas(flag-1,:);
end % if i == 1
   end % if j>2
...
NL_check = abs(NL_delta_disp(i,3)) - NL_Lim*abs(NL_delta_disp(i,2));
    if NL_check < 0
            k_def_NL = 0;   
    else
...
if abs(mean(i_std)) < std_check_limit      % creating reset of standard deviation form current mean if convergence limit reached
    ...
    if limit_pass > max_limit_pass              % Disregarding reset if max limit pass reached
        ...
        areas(:,ij) = i_mean(ij) + i_std_reset(ij)*randn(pop,1); % i_std_reset variable resets stand. dev from mean
         ...
         areas(1,:) = fittest_areas(flag-1,:); % Carrying fittest solution to next iteration for reset
...
if j > iterations*0.75 % Limiting forced area change until after iterations*X%
    limit_pass = max_limit_pass - 2; % stopping convergence resets after forced area reductions
    
if min_areas(:,ij) < areas_limit % setting if loop for min areas
                                 % if min area below limit (1e-7m^2 =
                                 % 0.1mm^2) then set area to almost zero
...
axis([0,j,0,0.0003])               % Limiting axis to relevent values
xlabel('Iterations')
