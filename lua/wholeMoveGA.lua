-- this uses multipoint crossover but only swaps whole moves
...
local function get_fitness(wt)
   ...
   -- make sure tori was not disqualified
   if wt == 2 then
...
local function multipoint_crossover(p, q)   
    noOfPoints = moves
    prevPoint =1
    c,d = {},{}
...
local function generational_replace(bestPop)
   ...
   while newPopsize < popSize do
	  ...
	  jVals1, jVals2 = multipoint_crossover(bestPop[ind1].jointVals,bestPop[ind2].jointVals)
