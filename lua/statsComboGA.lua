...
local name = "combo"              -- name for run of experiment
...
local disqualification =false      --award no points if player is disqualified 
...
local function write_pop()
   local currentPop = write_file(popFile)
...
local function check_best()  
   if population[1].fitness > bestIndiv.fitness then
      ...
      local indivString = bestIndiv.fitness..": "..table.concat(bestIndiv.jointVals,",")
      if bestFileExists == false then	 
	 local tmpFile = write_file(bestFile)
...
local function trunc_selection(pop)
   local cutoff = math.floor(table.getn(pop)*truncSize)
   local newPop = {}
   for i=1,cutoff do
...
--]]
function onepoint(p, q)
   local point = math.random(1,chromeLength)
   local c,d = {},{}
...
--crossover op that makes multiple randomly chosen swaps
function multipoint(p, q)
    local noOfPoints = moves
    local lowPoint =1
    local prevPoint =1
    local c,d = {},{}
    for i=1,noOfPoints do
       local highPoint = 22*i     
       local point = math.random(lowPoint,highPoint)
       if math.random() < crossoverProb then
...
--crossover op that swaps whole moves
function wholeMove(p, q)
   local noOfPoints = moves
   local prevPoint =1
   local c,d = {},{}
   for j=1,noOfPoints do        
      local point = 22*j          
      if math.random() < crossoverProb then	 
	 for i=prevPoint,point do
	    table.insert(d,p[i])
...
--crossover op that reorders whole moves
function moveOrder(p, q)
   local noOfPoints = moves
   local moveTable = {}
   local tmpMove1,tmpMove2 = {},{}
   local prevPoint =1
   local c,d = {},{}
   for j=1,noOfPoints do
      point = 22*j
      for i=prevPoint,point do
	 table.insert(tmpMove1,p[i])
	 table.insert(tmpMove2,q[i])
      end
      table.insert(moveTable,tmpMove1)
      table.insert(moveTable,tmpMove2)
      tmpMove1,tmpMove2 = {},{}
      prevPoint = point+1
   end
   --a quick shuffle!
   for i=1,table.getn(moveTable) do
      local swapIndex = math.random(1, table.getn(moveTable))
      local tmp = moveTable[swapIndex]
      moveTable[swapIndex] = moveTable[i]
      moveTable[i] = tmp
   end
   
   for i=1,noOfPoints do
      tmpMove1 = moveTable[i]
      tmpMove2 = moveTable[i+noOfPoints]
      for i =1,table.getn(tmpMove1) do
	 table.insert(c,tmpMove1[i])
	 table.insert(d,tmpMove2[i])
      end
   end
...
--replace the whole population(minus elites) with next generation
local function generational_replace(bestPop)
   ...
   local newPopsize = table.getn(newPop)
   local bestPopsize = table.getn(bestPop)
   while newPopsize < popSize do
       if newPopsize < popSize-1 then
	  ind1, ind2 = math.random(1,bestPopsize), math.random(1,bestPopsize)	  
	  jVals1, jVals2 = _G[crossOp](bestPop[ind1].jointVals,bestPop[ind2].jointVals)
...
local function get_average_fitness(pop)
   ...
   local averageFitness = totalFitness/popSize
   print("average fitness: "..averageFitness)
   return averageFitness
end

local function write_stats(statsFile)
   local genStats = generation.." "..bestIndiv.fitness.." "..get_average_fitness(population) 
