...
#include <stdlib.h>
#include <unistd.h>
const int MaxCountdown = 300;
const int MaxSaveFrames = 1000;
static char *oprefix;
static int tiltangle = 0;
static int grabCountdown = 100;
// end glgrab.c mods
...
void depth_cb(freenect_device *dev, void *v_depth, uint32_t timestamp)
{
  ...
  pthread_mutex_lock(&gl_backbuf_mutex);
  if (grabCountdown > 0) {
      if (grabCountdown % 10 == 0){
        printf("Countdown %d\n", grabCountdown);
      }
    grabCountdown--;
  } else {
    sprintf(imFname, "%s.bin", oprefix);
    printf("INFO: Saving depth image %s (T=%d)\n", imFname, timestamp);
    FILE *f = fopen(imFname, "w");
    fwrite(depth, FREENECT_FRAME_H*FREENECT_FRAME_W, sizeof(v_depth), f);
    fclose(f);
    pthread_mutex_unlock(&gl_backbuf_mutex);
    keyPressed(27,0,0);
  }
...
void *freenect_threadfunc(void *arg)
{
  ...
  freenect_set_tilt_degs(f_dev,tiltangle);
  freenect_set_led(f_dev,LED_GREEN);
...
int main(int argc, char **argv)
{
  ...
  oprefix = argv[1];
  sscanf(argv[2],"%d", &tiltangle);
  if(tiltangle < -30 || tiltangle > 30){
    printf("the angle must be between -30 and 30\n");
    exit(1);
  }
