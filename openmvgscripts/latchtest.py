...
def main(folder, detector):
    ...
    matches_dir = os.path.join(output_dir, detector+"matches")
    ...
    if not os.path.isfile(os.path.join(matches_dir, 'sfm_data.json')):
    ...
    global_reconstruct(matches_dir, output_dir)
    print_time("global time:", global_time)

    #sequential reconstruction
    sequential_time = time.time()
    sequential_reconstruct(matches_dir, output_dir)
...
def compute_matches(input_dir, matches_dir, camera_file_params, detector):
    ...
    print "2. LATCH Compute features"
    cmd = ["/home/jonathan/data/linuxapps/latchMVG/build/Linux-x86_64-Release/openMVG_main_ComputeFeatures", "-i", matches_dir+"/sfm_data.json",
...
def global_reconstruct(matches_dir, output_dir):
...
def sequential_reconstruct(matches_dir, output_dir):
    print "2. Compute matches"
    cmd = ["openMVG_main_ComputeMatches",  "-i", matches_dir+"/sfm_data.json",
           "-o", matches_dir, "-f", "1", "-n", "ANNL2"]
    run_cmd(cmd)

    reconstruction_dir = os.path.join(output_dir,"sequential")
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print ("Usage %s <image_dir> <sift|akaze_mldb|akaze_float>" % sys.argv[0])
        sys.exit(1)
