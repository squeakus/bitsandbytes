...
def obstacle_detector(frame):
    width, height = frame.shape
    box_width, box_height = int(width /4), int(height /4)
    prev_x, prev_y = 0, 0
    result = np.zeros((4,4))


    for idx, x in enumerate(range(box_width, width+1, box_width)):
        prev_y = 0
        for idy, y in enumerate(range(box_height, height+1, box_height)):
            box = frame[prev_x:x, prev_y:y]
            box_val = np.max(box)
            # box[box==1] = 0
            # box_val = np.mean(box[np.nonzero(box)])
            # if np.isnan(box_val):
            #     box_val = 0
            # box_val = int(box_val)
            result[idx,idy] = box_val
            frame[prev_x:x, prev_y:y] += box_val 
