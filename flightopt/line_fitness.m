function[A]=line_fitness(inputvector)
inputcell = num2cell(inputvector);
[fh,d,th1,th2] = inputcell{:};
Num=4;   % Number of buildings
points=xlsread('input3.xlsx','Sheet1');                                        % read the buidings vertices coordinates
mx=min(points(:,1));my=min(points(:,2));
maxx=max(points(:,1));maxy=max(points(:,2));
xcent=((maxx-mx)/2);                                                      % the centroid of the scanned area
ycent=((maxy-my)/2);
xss=points(:,1);
yss=points(:,2);
zss=points(:,3);
faces=xlsread('input3.xlsx','Sheet2');                                         % read the buildings topolog                                                                                                 
% the flight lines:
theta1=th1;                                                                   % angle with the positive x axis
theta2=th2;                                                                    % the angle for the perpindicular lines
d=d;                                                                        % distance for the center line from the centroid                                                                    % spacing between scan lines
[ag1 bg1 cg1]=Line_par(xcent,ycent,theta1,d);
[ag2 bg2 cg2]=Line_par(xcent,ycent,theta2,d);
zh=fh;
...
for i=1:15:length(points)
    ...
    lim2=lim-i+1;
    zground=min(points((i:i+lim2-1),3));                                      % to find the ground level of a building
    for j=i:(i+lim2-1)
    [xl yl]=lpoint(al,bl,cl,points(j,1),points(j,2));                         % to add the light source for each point
    points(j,4:5)=[xl yl];
    if points(j,3)==0
    points(j,6:8)=[points(j,1) points(j,2) points(j,3)];
    else
    [xg yg zg]=mproject(xl,yl,fh,points(j,1),points(j,2),points(j,3),0,0,1,-zground);  % to add the ground projection for each point.
    if isnan(zg)~=1
        zg=zground;
    end
...
for i=1:15:length(points)
    ...
    lim2=lim-i+1;
    points_cord(1:lim2,:,counter)=points(i:lim,:);                            % for each building, the points are stored [ point numbe XYZ Building number]
    counter=counter+1;
end
...
for j=1:10:length(faces)
   ...
   for jj=1:lim2
       sub_sub=sub(jj,:);
     faces_ind(jj,1:length(sub_sub),counter)=sub_sub;                         % for each building, the faces are stored as the index of the corners cordinates
   end
    counter=counter+1;
end
%% Processing
...
for i=1:Num
    ...
    kf(i,1:length(kftem))=kftem';                                             % to find the indeces for the foot print
end
...
size_tem=size(bldg_ocldr); % size_tem(2) is the maximum number of posible shadows to be projected on the surface
