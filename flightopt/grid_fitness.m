%% Optmize 
% calculates the shadow size for a given a flight path
%% Input
% Flight height,distance, flightline?, angle?, 
% spacing between lines?, number of lines
%% Returns
% The shadow size in metres cubed
%%

function[A]=grid_fitness(fh,d,th1,th2,sg,ng)
Num=4;   % Number of buildings
% sheet 1 contains the buildings vertex coordinates
...
yss=points(:,2);
zss=points(:,3);
% Sheet2 contains the buildings topology denoted as faces
...
for linnes=1:tem_line
    points=[xss yss zss];
    % create NaN array with the expected size for the points array
    points_cord=NaN(15,8,Num);
    % create NaN array with the expected size for the faces indeces
    faces_ind=NaN(10,8,Num);
