/*
    This program performs finite element analysis on a tetrahedron by reading in
...
int teConjPassemble(double *, int *, double *, int *, MATL *, double *, double *);

int tePassemble(int *, double *, double *, int *, MATL *, double *, SDIM *,
	double * );

int te2Passemble(int *, double *, double *, int *, MATL *, double *, SDIM *,
	double *, double *, double * );

int teFMassemble(int *, double *, double *, int *, double *, double *, MATL *,
	double *, double *);
...
/* The variables below are not really used.  They are only needed because
   some of the subroutines I use are from the linear tetrahedron code, which
...
*/
	...
	printf("tetrahedron structural data? (example: tespring4)\n");
	scanf( "%30s",name);
...
/*   o4 contains dynamic data   */
	...
	if( o4 == NULL ) {
		printf("Can't find file nteinput\n");
		exit(1);
	}
	else
	{
...
/* For the XYZI integers */
	...
	stress_read_flag = 1;
	check = tereader( bc, connect, coord0, el_matl, force, matl, name, o1,
...
/* The mass matrix */
	...
	check = teFMassemble(connect, coord, coordh, el_matl, force, mass, matl, U, Voln );
	if(!check) printf( "Problems with teFMassemble \n");
...
/* The code below uses te2Passemble which combines tePassemble and
   teConjPassemble.  I really only need the functionality of tePassemble
   and not teConjPassemble to calculate P_global, so it should have been
   more efficient to only call tePassemble, but an analysis using the
...
/*
		printf("\n %5d  %16.8e %14.6f %14.6f %14.6f",
			 iteration, timer, *(P_global+120), *(P_global+123), *(P_global+124));
		printf("\n %5d  %16.8e %14.6f %14.6f %14.6f",
			 iteration, timer, *(P_global+2), *(P_global+3), *(P_global+8));
*/

/*
		check = tePassemble( connect, coord, coordh, el_matl, matl, P_global,
		    stress, dU);
		if(!check) printf( "Problems with tePassemble \n");
*/
		check = te2Passemble( connect, coord, coordh, el_matl, matl, P_global,
		    stress, dU, P_global_CG, p);
		if(!check) printf( "Problems with te2Passemble \n");
...
/* The code below uses te2Passemble which combines tePassemble and
   teConjPassemble.  There is also an additional
...
/*
   Within te2Passemble, stress is updated, but there is no need to caculate
...
/* The reason tePassemble was called(before I wrote "te2Passemble" is to do stress updating rather
...
/*
	    check = teConjPassemble( A, connect, coord0, el_matl, matl, P_global_CG, U);
	    if(!check) printf( "Problems with teConjPassemble \n");
...
/* Count the number of times a particular node is part of an element */
		    ...
		    *(node_counter + node) += 1.0;
		    ...
		    hydro = (stress[k].xx + stress[k].yy +
			stress[k].zz)*Pois/Emod;
		    strain[k].xx = stress[k].xx/(2.0*G) - hydro;
		    strain[k].yy = stress[k].yy/(2.0*G) - hydro;
		    strain[k].zz = stress[k].zz/(2.0*G) - hydro;
		    strain[k].xy = stress[k].xy/G;
		    strain[k].zx = stress[k].zx/G;
		    strain[k].yz = stress[k].yz/G;
...
/* Calculate the principal strians */
		    ...
		    *(invariant+1) = xxyy +
			strain[k].yy*strain[k].zz +
			strain[k].zz*strain[k].xx - yzsq - zxsq - xysq;
		    *(invariant+2) =
			- xxyy*strain[k].zz -
...
/* Calculate the principal stresses */
		    ...
		    *(invariant+1) = xxyy +
			stress[k].yy*stress[k].zz +
			stress[k].zz*stress[k].xx - yzsq - zxsq - xysq;
		    *(invariant+2) =
			- xxyy*stress[k].zz -
...
/* Determine which elements are on the surface */
	...
	check = tewriter ( bc, connect, coord0, el_matl, force, id, matl,
