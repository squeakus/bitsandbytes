/*
    This program performs finite element analysis on a triangle by reading in
...
int trConjPassemble(double *, int *, double *, int *, double *, MATL *,
	double *, double *);

int trPassemble(int *, double *, double *, int *, MATL *, double *, SDIM *,
	double * );

int tr2Passemble(int *, double *, double *, int *, double *, double *, MATL *,
	double *, SDIM *, double *, double *, double * );

int trFMassemble( int *, double *, double *, int *, double *, double *, double *,
...
int main(int argc, char** argv)
{
	...
	XYZI *mem_XYZI;
	int *mem_int, sofmi, sofmf, sofmSDIM_node, sofmXYZI, sofmSDIM, ptr_inc;
	MATL *matl;
	double *mem_double;
...
/* The variables below are not really used.  They are only needed because
   some of the subroutines I use are from the linear triangle code, which
...
*/
	...
	printf("triangle structural data? (example: trtire2)\n");
	scanf( "%30s",name);
...
/*   o4 contains dynamic data   */
	...
	if( o4 == NULL ) {
		printf("Can't find file ntrinput\n");
		exit(1);
	}
	else
	{
...
/* For the doubles */
	sofmf=3*sdof + 7*dof + 2*numel + numnp + 2*numel*nsdsq;
...
/* For the doubles */
	...
	localh_xyz=(mem_double+ptr_inc);        ptr_inc += numel*nsdsq;
...
/* For the XYZI integers */
	...
	stress_read_flag = 1;
	check = trreader( bc, connect, coord0, el_matl, force, matl, name, o1,
	...
	check = local_vectors_triangle( connect, coord0, local_xyz );
	if(!check) printf( " Problems with local_vectors \n");
	check = local_vectors_triangle( connect, coord0, localh_xyz );
	if(!check) printf( " Problems with local_vectors \n");
...
/* The mass matrix */
	...
	check = trFMassemble(connect, coord, coordh, el_matl, force, mass, local_xyz,
		localh_xyz, matl, U, Area0 );
	if(!check) printf( "Problems with trFMassemble \n");
...
/* The code below uses tr2Passemble which combines trPassemble and
   trConjPassemble.  I really only need the functionality of trPassemble
   and not trConjPassemble to calculate P_global, so it should have been
   more efficient to only call trPassemble, but an analysis using the
...
/*
		check = trPassemble( connect, coord, coordh, el_matl, matl, P_global,
		    stress, dU);
		if(!check) printf( "Problems with trPassemble \n");
*/
		check = tr2Passemble( connect, coord, coordh, el_matl, local_xyz, localh_xyz,
		    matl, P_global, stress, dU, P_global_CG, p);
		if(!check) printf( "Problems with tr2Passemble \n");
...
/* The code below uses tr2Passemble which combines trPassemble and
   trConjPassemble.  There is also an additional
...
/*
   Within tr2Passemble, stress is updated, but there is no need to caculate
...
/* The reason trPassemble was called(before I wrote "tr2Passemble" is to do stress updating rather
...
/*
	    check = trConjPassemble( A, connect, coord0, el_matl, local_xyz, matl,
		P_global_CG, U);
	    if(!check) printf( "Problems with trConjPassemble \n");
 ...
 */
	    ...
	    analysis_flag = 2;
	    memset(p,0,dof*sof);
	    check = trKassemble(A, connect, coord, el_matl, p, id, idiag, K_diag,
...
/* Count the number of times a particular node is part of an element */
		...
		hydro = (stress[k].xx + stress[k].yy)*Pois/Emod;
		strain[k].xx = stress[k].xx/(2.0*G) - hydro;
		strain[k].yy = stress[k].yy/(2.0*G) - hydro;
		strain[k].xy = stress[k].xy/G;
...
/* Determine which elements are on the surface */
	...
	check = trwriter ( bc, connect, coord0, el_matl, force, id, matl,
