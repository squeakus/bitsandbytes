/*
    This program performs finite element analysis on a wedge by reading in
...
int weConjPassemble(double *, int *, double *, int *, MATL *, double *, double *);

int wePassemble(int *, double *, double *, int *, MATL *, double *, STRESS *,
	double * );

int we2Passemble(int *, double *, double *, int *, MATL *, double *, STRESS *,
	double *, double *, double * );

int weFMassemble(int *, double *, double *, int *, double *, double *, MATL *,
	double *);
...
/* The variables below are not really used.  They are only needed because
   some of the subroutines I use are from the linear wedge code, which
...
/* Create local shape funcions */
	...
	printf("wedge structural data? (example: spring2a)\n");
	scanf( "%30s",name);
...
/*   o4 contains dynamic data   */
	...
	if( o4 == NULL ) {
		printf("Can't find file nweinput\n");
		exit(1);
	}
	else
	{
...
/* For the SDIM doubles */
	...
	stress_read_flag = 1;
	check = wereader( bc, connect, coord0, el_matl, force, matl, name, o1,
...
/* Initializing the data for the Volume */
	...
	check = weVolume( connect, coord0, Vol0);
...
/* The mass matrix */
	...
	check = weFMassemble(connect, coord, coordh, el_matl, force, mass, matl, U );
	if(!check) printf( "Problems with weFMassemble \n");
...
/* The code below uses we2Passemble which combines wePassemble and
   weConjPassemble.  I really only need the functionality of wePassemble
   and not weConjPassemble to calculate P_global, so it should have been
   more efficient to only call wePassemble, but an analysis using the
...
/*
		check = wePassemble( connect, coord, coordh, el_matl, matl, P_global,
		    stress, dU);
		if(!check) printf( "Problems with wePassemble \n");
*/
		check = we2Passemble( connect, coord, coordh, el_matl, matl, P_global,
		    stress, dU, P_global_CG, p);
		if(!check) printf( "Problems with we2Passemble \n");
...
/* The code below uses we2Passemble which combines wePassemble and
   weConjPassemble.  There is also an additional
...
/*
   Within we2Passemble, stress is updated, but there is no need to caculate
...
/* The reason wePassemble was called(before I wrote "we2Passemble" is to do stress updating rather
...
/*
	    check = weConjPassemble( A, connect, coord0, el_matl, matl, P_global_CG, U);
	    if(!check) printf( "Problems with weConjPassemble \n");
...
/* Determine which elements are on the surface */
	...
	check = wewriter ( bc, connect, coord0, el_matl, force, id, matl,
