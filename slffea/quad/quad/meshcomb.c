/*
    ...
    using quad elements.  There are also the 2 mesh generators
...
int main(int argc, char** argv)
{
	...
	if( comb_backing_flag) numel += 2*vert_el_num*( 2*horiz_el_num-1 );
	...
	fprintf( o1, "matl no., E modulus, Poisson Ratio, density, thickness\n");
	for( i = 0; i < nmat; ++i )
	{
	   fprintf( o1, "%3d ",0);
	   fprintf( o1, " %9.4f %9.4f     %9.4f  %9.4f\n ",10000000.0, 0.3, 2.77e3, 1.0);
	}
	...
	for( i = 0; i < vert_el_num; ++i )
	{
		for( j = 0; j < 2*horiz_el_num; ++j )
		{
			fprintf( o1, "%4d %4d %4d %4d %4d %4d",dum,
				i*8*horiz_el_num + j,
				i*8*horiz_el_num + j + 2*horiz_el_num,
				i*8*horiz_el_num + j + 6*horiz_el_num,
		...
		for( j = 0; j < horiz_el_num-1; ++j )
		{
			fprintf( o1, "%4d %4d %4d %4d %4d %4d",dum,
				...
				i*8*horiz_el_num + 2*j + 1 + 2*horiz_el_num,
				i*8*horiz_el_num + 2*j + 2 + 2*horiz_el_num,
		...
		for( j = 0; j < horiz_el_num; ++j )
		{
			fprintf( o1, "%4d %4d %4d %4d %4d %4d",dum,
				...
				i*8*horiz_el_num + 2*j + 6*horiz_el_num,
				i*8*horiz_el_num + 2*j + 1 + 6*horiz_el_num,
		...
		for( j = 0; j < 2*horiz_el_num; ++j )
		{
			fprintf( o1, "%4d %4d %4d %4d %4d %4d",dum,
				i*8*horiz_el_num + j + 4*horiz_el_num,
				i*8*horiz_el_num + j + 6*horiz_el_num,
				i*8*horiz_el_num + j + 10*horiz_el_num,
...
/* The code below represents the elements that are on the back of the combs. */
	...
	if( comb_backing_flag)
	{
	    for( i = 0; i < vert_el_num; ++i )
	    {
		for( j = 0; j < 2*horiz_el_num-1; ++j )
		{
			fprintf( o1, "%4d %4d %4d %4d %4d %4d",dum,
				i*8*horiz_el_num + j,
				i*8*horiz_el_num + j + 1,
				i*8*horiz_el_num + j + 1 + 4*horiz_el_num,
				i*8*horiz_el_num + j + 4*horiz_el_num,
				0);
			fprintf( o1, "\n");
			++dum;
			fprintf( o1, "%4d %4d %4d %4d %4d %4d",dum,
				i*8*horiz_el_num + j + 4*horiz_el_num,
				i*8*horiz_el_num + j + 1 + 4*horiz_el_num,
				i*8*horiz_el_num + j + 1 + 8*horiz_el_num,
				i*8*horiz_el_num + j + 8*horiz_el_num,
				0);
			fprintf( o1, "\n");
			++dum;
	...
	if(flat_comb_flag)  /* These are the nodes for the flat comb. */
	{
	    ...
	    A = 2.0*rad_length;
	    B = el_length;
	    for( i = 0; i < 2*vert_el_num + 1; ++i )   /* Nodes on inner layer */
	    {
		fdum1 = 0.0;
		for( j = 0; j < horiz_el_num; ++j )
		{
		...
		fdum1 = 0.0;
		for( j = 0; j < horiz_el_num; ++j )   /* Nodes on outer layer */
		{
			fdum2 = fdum3;
			Y = fdum3;
			X = fdum1 + fdum4;
	...
	else  /* These are the nodes for the cylindrical comb. */
	{
		...
		{
			Y = fdum3;
			X = (comb_R + comb_thick)*sin(ang1 + ang4);
			Z = (comb_R + comb_thick)*cos(ang1 + ang4);
			fprintf( o1, "%d ",dum);
			fprintf( o1, "%9.4f   %9.4f  %9.4f ", X, Y, Z);
			fprintf( o1, "\n");
			++dum;
...
/* Ending vertical side */
		...
		fprintf( o1, "%4d %14.6e\n", dum, 0.0);
		dum = 2*vert_el_num*4*horiz_el_num + i + 2*horiz_el_num;
		fprintf( o1, "%4d %14.6e\n", dum, 0.0);
	    }
	    fprintf( o1, "%4d\n ",-10);
...
*/
		...
		{
		    dum = (i + vert_el_num - center_shift/2)*4*horiz_el_num +
			horiz_el_num - center_shift/2 + j;
		    fprintf( o1, "%4d %14.6e %14.6e %14.6e\n", dum, 0.0, 0.0, 100000.0);
		}
	    }
	    fprintf( o1, "%4d\n ",-10);
	}
