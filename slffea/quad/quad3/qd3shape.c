...
int qd3shg( double *dcdx, double *det, int el, double *shl, double *shg,
	double *xl, double *Area)
{
/*
     This subroutine calculates the global shape function derivatives for
     an electromagnetic quad element at the gauss points.
 ...
 ....  CALCULATE GLOBAL DERIVATIVES OF SHAPE FUNCTIONS AND
       ...
       *(dcdx+nsdsq*k)      = d(XI)/dx
       *(dcdx+nsdsq*k + 1)  = d(XI)/dy
       *(dcdx+nsdsq*k + 2)  = d(ETA)/dx
       *(dcdx+nsdsq*k + 3)  = d(ETA)/dy
       ...
       *(xs+2*i+j) = JACOBIAN MATRIX
       ...
       num_int    = NUMBER OF INTEGRATION POINTS, EQ.8 
...
/* The inverse of the jacobian, dc/dx, is calculated below and
   stored in dcdx
*/
     	   temp=*(xs);
	   *(xs) = *(xs + 3)/(*(det+k));
...
{
/*
     This subroutine calculates the global shape function for
