...
int rout(int i, int j, Mat mag, Mat &edge, Mat dir, link_pix* &pix_now, const link_seg* seg_now)
{
	...
	pix_now = pix_now->prev;
	pix_now->ifend = 0;                                     //�ߵ�ͷ������Ե������һ����
	link_pix* end = pix_now;
	pix_now = seg_now->addr;                                //�ص���ʼ�ĵ�
	i = pix_now->x;
	j = pix_now->y;
	edge.at<uchar>(i, j) = 128;
	while (mag.at<short>(i, j) > 0 && edge.at<uchar>(i, j) != 255)
...
int main(int argc, char* argv[])
{
	...
	seg_now->next = (link_seg*)malloc(sizeof(link_seg));
	...
	for (int i = 1; i < mag.rows; i++)
	{
		...
		{
			if (edge.at<uchar>(i, j) == 128)                      //�����ҵ���ê����������һ��
			{
				seg_now->addr = (link_pix*)malloc(sizeof(link_pix));  //�����ñ�Ե�еĵ�һ�����ص�
				pix_now = seg_now->addr;                           //����ָ��ָ���ղŴ����ĵ�һ����
				...
				if (k > shortestline)
				{
					...
					seg_now->next = (link_seg*)malloc(sizeof(link_seg));            //�����µı�Ե
					seg_now = seg_now->next;                                     //ָ��ָ��ȥ
					seg_now->next = NULL;
					seg_now->addr = NULL;
				}
				else
	...
	imshow("arc_candi", BGR_edge);                          //��ʾ���ܵ�Բ��
	waitKey(0);
	...
	while (arc_list.size() != 0)
	{
		link_circle circle;                                            //����һ����Բ
		circle.arc_chain = new vector<link_arc>;
		circle.pix_chain = new vector<link_pix*>;
		float radius = arc_list[0].r;                                   //��¼��һ��Բ����Բ�ĺͰ뾶
		Point center = arc_list[0].o;

		vector<link_arc>::iterator iter = arc_list.begin();
		(*circle.arc_chain).push_back(*iter);                           //����һ��Բ�������½���Բ�ĵ�һ��λ��
		iter = arc_list.erase(iter);                                          //���Ѳ�����Բ��ɾ��
		vector<link_arc>::iterator end = arc_list.end();
	...
	{
		if (circle_list_iter->pix_chain->size() < pi*0.75*circle_list_iter->r)                  //������Ȧ���Żص�Բ���б�����һ������Բ
		{
...
/*	paixu(arc_list);
	...
	while (arc_list.size() != 0)
	{
		...
		vector<link_arc>::iterator iter = arc_list.begin();
		(*ellips.arc_chain).push_back(*iter);                           //����һ��Բ�������½�����Բ�ĵ�һ��λ��
		iter = arc_list.erase(iter);                                          //���Ѳ�����Բ��ɾ��
		vector<link_arc>::iterator end = arc_list.end();
