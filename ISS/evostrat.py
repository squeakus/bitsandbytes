...
class Evostrategy:
    ...
    def __init__(self, genome_size, pop_size):
        ...
        self.max_codon_value = 360
        self.min_codon_value = 0
        self.pop = []
        self.initialise_pop()
        self.gen_count = 0
        self.mut_rate = 0.1
    ...
    def create_indiv(self):
        """initialise an individual with rotation values"""
        indiv = [random.randint(self.min_codon_value, self.max_codon_value) 
                 for _ in range(0, self.genome_size)]
        return indiv
    ...
    def mutate(self,indiv):
        ...
        for i in range(len(mutant)):
            if random.random() < self.mut_rate:
                mutant[i] = random.randint(self.min_codon_value, 
                                           self.max_codon_value)
        return mutant

    def addsubtract(self,indiv):
        """mutate each codon with a given probability"""
        mutant = list(indiv)
        for i in range(len(mutant)):
            if random.random() < self.mut_rate:
    ...
    def adapt_mutation(self):
        ...
        # 1/5th rule
        if success_rate > 0.2:
            print "increasing mutrate"
            self.mut_rate = self.mut_rate * 2
        else:
            print "decreasing mutrate"
            self.mut_rate = self.mut_rate / 2
        self.success_mut = 0
    ...
    def iterate(self, children):
        ...
        for parent in self.pop:
            mutated_genome = self.addsubtract(parent['genome'])
