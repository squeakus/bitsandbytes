...
class graph(nx.Graph):
    ...
    def nearest_node(self,x,y,z):
        pos = [x,y,z]
        distSort = lambda k: euclidean_distance(self.node[k]['xyz'], pos)
        nn = min(self.node, key=distSort)
        distance = euclidean_distance(self.node[nn]['xyz'], pos)
        #print "nodeId:",str(nn)
        #print "coords:",self.node[nn]['xyz']
    ...
    def connect_loop(self, nodeList):
        nodeList.sort()
        for i in range(len(nodeList) - 1):
            self.add_edge(nodeList[i], nodeList[i+1])
            self.save_picture()
        self.add_edge(nodeList[0], nodeList[-1])
        self.save_picture()

    def connect_all_nodes(self, nodeList):
    ...
    def dist_connect(nodeList,length):
        for nodeA in nodeList:
            for nodeB in nodeList:
                if int(euclidean_distance(nodeA,nodeB))==length:        
                    self.add_edge(nodeA,nodeB)
                    self.save_picture()
    ...
    def copy_and_offset_with_mirror(self, offset, mirror=False):
        ...
        new_graph.save_picture()
        self.clear()

        #only add nodes connected to edges
        for edge in new_graph.edges_iter(data=True):
            if edge[0]==edge[1]:
        ...
        # testEdge = (132, 134, {})
        # new_graph.add_edge(132,134,{'elem':"one"})
        # new_graph.add_edge(134,132,{'elem':"two"})
        # for edge in new_graph.edges_iter(data=True):
        #     if edge[0] == testEdge[0] and  edge[1] == testEdge[1]:
        #         print "found it!",str(edge)
