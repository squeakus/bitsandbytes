...
if noforwardfrominput == 0,
    ...
    B = PTS';
    spparms('spumoni',1);
    mlweights = mldivide(A,B);  % another way to write A\b
    inweights = linsolve([AH', IS'],PTS')';
    newweights = ([AH', IS'] \ PTS')';
    net.weightsForward = ([AH', IS'] \ PTS')';
    disp(size([AH', IS']));
    
    disp(size(net.weightsForward))
    residual =[AH', IS']*net.weightsForward'-PTS';
else
