...
int regulate(int conc_count, double concs[conc_count],
  ...
  for(int i = 0; i < tf_count; i++){
    ...
    if(concs[tf_genes[i]] < 0){
      below_zero = 1;
    }

    if(concs[tf_genes[i]] < 1e-10){concs[tf_genes[i]] = 1e-10;}
  } 
  ...
  for(int i = 0; i < p_count; i++){
    ...
    if(concs[p_genes[i]] < 1e-10){concs[p_genes[i]] = 1e-10;}
  } 
  return below_zero;
}

int regulateSync(int conc_count, double concs[conc_count],
		 int rows, int cols, double weights[rows][cols],
		 int tf_count, int tf_genes[tf_count],
		 int p_count, int p_genes[p_count],
		 int delta, double e_total){
