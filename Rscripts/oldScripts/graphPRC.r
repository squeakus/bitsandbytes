...
xoRates <-c("0.0","0.9")
experimentDir <- ""
mutationOps <-c("varmut")
mutRates <-c("0.01","0.1","0.15","0.2")
graphAll <- function(experiment)
{
  baseDir <- "/Users/jbyrne/MYGEVA/ExperimentManager/galapagosResults"	
  ...
  {  	
  	...
  	for(xoRate in xoRates)
  	{
  	  ...
  	  {	
  	  	compareArray <- matrix(,nrow=51,ncol=length(mutationOps))
        expCounter <-1
