...
mutRates <-c("0.1","0.01","0.02")
noOfGenerations <- 50 +1

graphAll <- function(experiment)
{
  baseDir <- "/Users/jonathanbyrne/ExperimentManager"	
  ...
  {  	
  	...
  	for(depth in depths)
  	{
        ...
        for(mutationOp in mutationOps)
    	  {
            experimentName <-paste(experiment,problem,"depth5","xo_0.0",mutationOp,"m",mutRate, sep="_")	
		...
		noOfRuns <- length(files)
		   print(experimentName)
        	...
        	compareArray[,expCounter] <- meanBest			
  	       ...
  	       fileName <- paste(fileName,"Depth",depth,"Mut",mutRate,sep="_")
    	   fileName <- paste(fileName, "eps", sep=".")
    	   graphTitle <- paste(problem,"Depth",depth,"Mut",mutRate,sep=" ")
    	   print(fileName)
