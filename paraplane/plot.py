...
def fionngraph(names):    
    folders = ["evolved/", "random/"]
    names = ['run10','run2','run3','run4','run5','run6','run7','run8','run9','run1']
    liftaverages, dragaverages = [],[]
    liftdevs, dragdevs = [],[]
...
def fionncompare(moo):
    folders = ["evolved/", "random/"]
    names = ['run10','run2','run3','run4','run5','run6','run7','run8','run9','run1']
    
    resultsfiles,colors, handles = [],[],[]
    for name in names:
        resultsfolder = folders[0]+name+"/"
        for filename in os.listdir(resultsfolder):
            if filename.startswith('gen') and filename.endswith('.dat'):
                resultsfiles.append(resultsfolder+filename)
    ...
    resultsfolder = folders[0]+names[0]+"/"
    for filename in os.listdir(resultsfolder):
        if filename.startswith('gen') and filename.endswith('.dat'):
            for idx, name in enumerate(names):
                resultsfile =folders[0]+name+"/"+filename
            ...
            for idx, name in enumerate(names):
                resultsfile =folders[1]+name+"/"+filename
                if os.path.exists(resultsfile):
                    color = 'blue'
                    handle = scatterplot(resultsfile, color, lift_range, 
                                         drag_range, name)
...
def plotruns(names):
    ...
    resultsfolder = RESFOLDER+names[0]+"/"
    resultsfiles = []
    for filename in os.listdir(resultsfolder):
        if filename.startswith('gen') and filename.endswith('.dat'):
            for idx, name in enumerate(names):
                resultsfile =RESFOLDER+name+"/"+filename
                if os.path.exists(resultsfile):
